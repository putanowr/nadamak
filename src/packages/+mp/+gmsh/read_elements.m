...
function [elements] = read_elements(fid_or_name, version)
  arguments
      fid_or_name
...
function [elements] = local_read_elements_ver_4(fid_or_name)
  ...
  elements = mp.SharedCellArray(numElements); %preallocate memory
  elemId = 0;
...
function [elements] = local_read_elements_ver_2(fid_or_name)
  ...
  tline = fgetl(fid);
  n = sscanf(tline, '%d');
  elements = mp.SharedCellArray(n); %preallocate memory
  for i = 1:n
    tline = fgetl(fid);
