function [selector] = makeBcSelector(bcRegistry)
  % Return selector for boundary conditions. Selector is a structure
  % that maps region names to numeric tags. The values of numeric tags
  % depend on the boundary conditions specified on the regions.
  rn = bcRegistry.regionNames();
  selector = struct();
  for regionName = rn
    bc = bcRegistry.get(regionName{:},'Displacement');
    selector.(regionName{:}) = mp.exports.flagshyp.bc2tag(bc.type);
  end
end

